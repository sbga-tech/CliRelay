...
Docker Compose 是 CliRelay 推荐的安装方式。仓库内的 `docker-compose.yml` 会启动 CliRelay、PostgreSQL 15、Redis 7 和 updater sidecar。本地使用时 `.env` 不是必需的，compose 文件自带可运行默认值；生产环境建议显式设置 `CLIRELAY_UPDATER_TOKEN`、`CLIRELAY_POSTGRES_PASSWORD` 和各类挂载路径。
...
CliRelay 通过 Ent ORM 使用 PostgreSQL 15+ 作为运行时主数据库。Redis 7+ 只负责缓存、锁、限流、队列和可重建快照。仓库内的 Docker Compose 会自动启动这两个服务，并注入容器内连接地址。

如果旧 Docker 部署还停留在 SQLite-only compose，在 `/manage/system` 里触发在线更新时，updater 会先升级 `docker-compose.yml` 和 `.env`，再执行 `docker compose up -d --remove-orphans`。升级会补上 PostgreSQL/Redis 服务，保留原应用服务，写入缺失的生成配置，然后由容器 entrypoint 导入旧 SQLite 数据。SQLite 文件会原样保留。
